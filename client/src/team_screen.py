...
from data_client import get_team
...
class TeamScreen(Screen):
    def __init__(self, team_id: int, **kwargs):
        ...
        self.bottom_layout = TeamOnFieldLayout(self.team["id"], size_hint_y=0.7)
    ...
    def on_team_press(self, instance):
        if hasattr(self, 'bottom_layout'):
            self.screen_layout.remove_widget(self.bottom_layout)
        self.bottom_layout = PlayersTable(self.team["id"], size_hint_y=0.7)
        self.screen_layout.add_widget(self.bottom_layout)
        print('team')
